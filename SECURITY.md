...
havliand_agent and also where the boundaries are.

In general havliand_agent is a coding agent that runs locally within the security boundary
...
havliand_agent treats the local user account and files writable by that account as inside
the same trust boundary as the HavliandAgent process itself.  If an attacker can modify files
under the user's home directory, workspace, shell startup files, environment, or
HavliandAgent configuration, they can generally influence havliand_agent or other local developer tools.
Reports that depend on such prior local write access are not security
vulnerabilities unless they demonstrate how havliand_agent grants that write access or crosses
an operating-system privilege boundary.

havliand_agent relies on users installing trustworthy extensions and loading trustworthy
skills and only to use havliand_agent within trusted repositories.  This is because files
...
If you believe you found a security vulnerability in havliand_agent or another package in
...
- Local code execution or sandboxing behavior (the HavliandAgent coding agent intentionally does not have a sandbox)
- Behavior of HavliandAgent extensions or skills installed by the user
...
- Public internet exposure of a HavliandAgent installation
...
- Reports requiring the ability to create, modify, delete, or replace files,
  ...
  user-controlled local state on the target machine. This includes `~/.havliand_agent`,
  `~/.havliand_agent/agent/models.json`, workspace files, `AGENTS.md`, skills, extensions,
  extension configuration, dotfiles, and files synchronized through NFS, roaming
  profiles, or dotfile managers, unless the report shows how havliand_agent itself grants
  that access.
- Issues caused by intentionally weakened user configuration.
- Resource/DOS claims that require trusted local input/config against the HavliandAgent coding agent.
...
For example, a report showing that malicious contents written to a trusted havliand_agent
configuration file cause havliand_agent to execute commands, load attacker-controlled tools,
...
affected and that the issue is reachable through havliand_agent.  For exposed-secret reports,
include evidence that the credential is owned by this project or grants access
to project-operated infrastructure or services.
