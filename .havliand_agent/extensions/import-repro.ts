/**
 * Import a havliand_agent session shared as a gist by the issue-analysis CI workflow
 ...
 *   /ir https://havliand_agent.dev/session/#b4d100022aefb12f25dd2d8485e0a82a
 *   /ir https://github.com/kk7041/havliand_agent/issues/123
 *
 *   havliand_agent "/ir <gist-id>"
 */

import { Buffer } from "node:buffer";
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { basename, isAbsolute, join, resolve } from "node:path";
import type { ExtensionAPI, ExtensionCommandContext } from "@havliand_agent/coding-agent";
...
function parseRef(
	...
	throw new Error(`expected a gist ID, gist URL, havliand_agent.dev share URL, issue URL, .html file, or .jsonl file: ${ref}`);
}
...
function decodeExportedHtml(html: string): { header: SessionHeader; jsonl: string } {
	...
	if (!match) throw new Error("HTML does not contain embedded havliand_agent session data");

	let data: unknown;
	try {
		data = JSON.parse(Buffer.from(match[1], "base64").toString("utf8"));
	} catch {
		throw new Error("embedded havliand_agent session data is not valid JSON");
	}
	...
	if (!header || header.type !== "session" || typeof header.id !== "string" || typeof header.cwd !== "string") {
		throw new Error("embedded havliand_agent session data has no valid session header");
	}
	if (!Array.isArray(sessionData.entries)) {
		throw new Error("embedded havliand_agent session data has no entries array");
	}
...
export default function (havliand_agent: ExtensionAPI) {
	havliand_agent.registerCommand("ir", {
		description: "Import a CI issue-analysis session from a gist ID, share URL, or issue URL and switch to it",
		handler: async (args: string, ctx: ExtensionCommandContext) => {
			const ref = args.trim();
			if (!ref) {
				ctx.ui.notify("Usage: /ir <gist-id | gist-url | havliand_agent.dev/session URL | issue URL>", "error");
				return;
			}
