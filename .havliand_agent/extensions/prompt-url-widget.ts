...
import { DynamicBorder, type ExtensionAPI, type ExtensionContext } from "@havliand_agent/coding-agent";
import { Container, Text } from "@havliand_agent/tui";
...
async function fetchAdvisoryMetadata(havliand_agent: ExtensionAPI, cwd: string, target: string): Promise<GhMetadata | undefined> {
	const advisoryRef = parseAdvisoryUrl(target) ?? (await readAdvisoryRefFromDraft(cwd, target));
	if (!advisoryRef) return undefined;

	try {
		const result = await havliand_agent.exec("gh", [
			"api",
...
async function fetchGhMetadata(
	...
	if (kind === "advisory") {
		return fetchAdvisoryMetadata(havliand_agent, cwd, target);
	}
	...
	try {
		const result = await havliand_agent.exec("gh", args);
...
export default function promptUrlWidgetExtension(havliand_agent: ExtensionAPI) {
	...
	const applySessionName = (ctx: ExtensionContext, match: PromptMatch, metadata?: GhMetadata) => {
		...
		const currentName = havliand_agent.getSessionName()?.trim();
		if (!currentName) {
			havliand_agent.setSessionName(desiredName);
			return;
		}
	...
	const updatePromptContext = (ctx: ExtensionContext, match: PromptMatch) => {
		...
		void fetchGhMetadata(havliand_agent, match.kind, match.target, ctx.cwd).then((meta) => {
	...
	havliand_agent.on("session_switch", async (_event, ctx) => {
