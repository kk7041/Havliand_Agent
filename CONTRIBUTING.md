# Contributing to havliand_agent

This guide exists to save both sides time.

## Philosophy

First things first: **havliand_agent's core is minimal**.

If your feature does not belong in the core, it should be an extension. PRs that bloat the core will likely be rejected.

havliand_agent's core exists to be minimal and to be extensible so that it can be influenced and manipulated by extensions.  Even hook points for extensions however should be well considered and discussed to avoid adding unmaintainable bloat and complex interactions.
...
If you use an agent, run it from the `havliand_agent` root directory so it picks up `AGENTS.md` automatically. Your agent must follow the rules and guidelines in that file.
...
quite a few are.  They can be found at [github.com/kk7041/havliand_agent/discussions](https://github.com/kk7041/havliand_agent/discussions/keyword/havliand_agent/).
