...
import { type HavliandAgentMessagesOptions, stream, streamSimple } from "../src/api/havliand_agent-messages.ts";
...
function createModel(baseUrl: string): Model<"havliand_agent-messages"> {
	return {
		id: "auto",
		name: "Radius Auto",
		api: "havliand_agent-messages",
		provider: "radius",
		baseUrl,
		reasoning: false,
		input: ["text"],
...
describe("havliand_agent-messages", () => {
	...
	it("appends debug=1 and reports response headers via onResponse", async () => {
		...
		const options: HavliandAgentMessagesOptions = {
			apiKey: "test-key",
			debug: true,
			onResponse: (response) => {
	...
	it("surfaces backend error responses with diagnostics", async () => {
		...
		expect(message.diagnostics?.[0]?.type).toBe("havliand_agent_messages_response_failure");
...
describe("havliand_agent-messages api registration", () => {
	it("is registered as a builtin api provider", async () => {
		const { getApiProvider } = await import("../src/compat.ts");
		expect(getApiProvider("havliand_agent-messages")).toBeDefined();
	});

	it("is a known api usable on models", () => {
		const api: Api = "havliand_agent-messages";
		expect(api).toBe("havliand_agent-messages");
	});
});
