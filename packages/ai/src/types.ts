...
import type { HavliandAgentMessagesOptions } from "./api/havliand_agent-messages.ts";
...
export interface ApiOptionsMap {
	...
	"havliand_agent-messages": HavliandAgentMessagesOptions;
}
...
export interface OpenAICompletionsCompat {
	...
	thinkingFormat?:
		...
		| "string-thinking"
		| "ant-ling";
	/** Kwargs to send as `chat_template_kwargs` when `thinkingFormat` is `chat-template`. Use `{ "$var": "thinking.enabled" }` or `{ "$var": "thinking.effort" }` for HavliandAgent-controlled thinking values. */
...
export interface Model<TApi extends Api> {
	...
	reasoning: boolean;
	/**
	 * Maps havliand_agent thinking levels to provider/model-specific values.
