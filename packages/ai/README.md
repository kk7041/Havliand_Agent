...
```bash
npm install @havliand_agent/ai
```

TypeBox exports are re-exported from `@havliand_agent/ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@havliand_agent/ai';
import { builtinModels } from '@havliand_agent/ai/providers/all';
...
```typescript
import { anthropicProvider } from '@havliand_agent/ai/providers/anthropic';
import { openaiProvider } from '@havliand_agent/ai/providers/openai';
import { openrouterProvider } from '@havliand_agent/ai/providers/openrouter';
import { amazonBedrockProvider } from '@havliand_agent/ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@havliand_agent/ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@havliand_agent/ai/providers/all';
...
Stored credentials (API keys entered interactively, OAuth tokens) live in a `CredentialStore` — one type-tagged credential per provider. havliand_agent-ai ships an in-memory default; apps inject persistent storage:

```typescript
import { createModels, type CredentialStore } from '@havliand_agent/ai';
...
API-key credentials use the same discriminator as havliand_agent's `auth.json` and can carry provider-scoped env/config values:
...
```typescript
import { Type, type Tool, StringEnum } from '@havliand_agent/ai';
...
```typescript
import { validateToolCall, type Tool } from '@havliand_agent/ai';
...
```typescript
import { builtinImagesModels } from '@havliand_agent/ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@havliand_agent/ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@havliand_agent/ai';
import { openAICompletionsApi } from '@havliand_agent/ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@havliand_agent/ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@havliand_agent/ai/api/openai-responses.lazy';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are havliand_agent thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`). Missing standard levels through `high` use provider defaults; `xhigh` and `max` are opt-in and require a non-null map entry. String values are sent to the provider, `null` marks a level unsupported, and maps may skip levels.
...
```typescript
import { stream } from '@havliand_agent/ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@havliand_agent/ai/api/<api-id>`.
...
interface OpenAICompletionsCompat {
  ...
  chatTemplateKwargs?: Record<string, string | number | boolean | null | { '$var': 'thinking.enabled' | 'thinking.effort'; omitWhenOff?: boolean }>; // chat_template_kwargs values; use $var for havliand_agent-controlled thinking values
...
```typescript
import { createModels, type Context } from '@havliand_agent/ai';
import { anthropicProvider } from '@havliand_agent/ai/providers/anthropic';
import { openaiProvider } from '@havliand_agent/ai/providers/openai';
import { googleProvider } from '@havliand_agent/ai/providers/google';
...
```typescript
import { createModels } from '@havliand_agent/ai';
import { anthropicProvider } from '@havliand_agent/ai/providers/anthropic';
...
- `@havliand_agent/ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@havliand_agent/ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@havliand_agent/ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@havliand_agent/ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@havliand_agent/ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@havliand_agent/ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@havliand_agent/ai/bedrock-provider';
...
Pass `env` in stream options to scope provider configuration to a request. Values in `env` are used before process environment variables for provider auth and configuration such as Cloudflare account IDs, Azure OpenAI settings, Vertex project/location, Bedrock settings, `HAVLIAND_AGENT_CACHE_RETENTION`, and `HTTP_PROXY`/`HTTPS_PROXY`.
...
```bash
npx @havliand_agent/ai login              # interactive provider selection
npx @havliand_agent/ai login anthropic    # login to specific provider
npx @havliand_agent/ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@havliand_agent/ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@havliand_agent/ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@havliand_agent/ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@havliand_agent/ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@havliand_agent/ai`.
