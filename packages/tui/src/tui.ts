...
export class TUI extends Container {
	...
	private showHardwareCursor = process.env.HAVLIAND_AGENT_HARDWARE_CURSOR === "1";
	private clearOnShrink = process.env.HAVLIAND_AGENT_CLEAR_ON_SHRINK === "1"; // Clear empty rows when content shrinks (default: off)
	...
	private doRender(): void {
		...
		const debugRedraw = process.env.HAVLIAND_AGENT_DEBUG_REDRAW === "1";
		const logRedraw = (reason: string): void => {
			if (!debugRedraw) return;
			const logPath = path.join(os.homedir(), ".havliand_agent", "agent", "havliand_agent-debug.log");
		...
		// Configurable via setClearOnShrink() or HAVLIAND_AGENT_CLEAR_ON_SHRINK=0 env var
		...
		for (let i = firstChanged; i <= renderEnd; i++) {
			...
			if (!isImage && visibleWidth(line) > width) {
				...
				const crashLogPath = path.join(os.homedir(), ".havliand_agent", "agent", "havliand_agent-crash.log");
				const crashData = [
		...
		if (process.env.HAVLIAND_AGENT_TUI_DEBUG === "1") {
