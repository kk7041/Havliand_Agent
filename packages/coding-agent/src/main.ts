...
import { type ImageContent, modelsAreEqual } from "@havliand_agent/ai";
import chalk from "chalk";
import { handleAgentPayCommand } from "./agentpay-cli.ts";
...
const EXTENSION_LOAD_FAILURE_HINT = 'Hint: Start without extensions using "havliand_agent -ne".';
...
export async function main(args: string[], options?: MainOptions) {
	resetTimings();
	const offlineMode = args.includes("--offline") || isTruthyEnvFlag(process.env.HAVLIAND_AGENT_OFFLINE);
	if (offlineMode) {
	...
	if (await handleAgentPayCommand(args)) {
	...
	if (await handlePackageCommand(args, { extensionFactories: options?.extensionFactories })) {
		...
		if (process.platform === "win32" && exitCode === 0 && args[0] === "update") {
			...
			// runs during teardown; let successful `havliand_agent update` drain naturally instead.
	...
	const startupBenchmark = isTruthyEnvFlag(process.env.HAVLIAND_AGENT_STARTUP_BENCHMARK);
	if (startupBenchmark && appMode !== "interactive") {
		console.error(chalk.red("Error: HAVLIAND_AGENT_STARTUP_BENCHMARK only supports interactive mode"));
		process.exit(1);
	}
