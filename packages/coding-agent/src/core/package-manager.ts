...
function isOfflineModeEnabled(): boolean {
	const value = process.env.HAVLIAND_AGENT_OFFLINE;
...
type SkillDiscoveryMode = "havliand_agent" | "agents";
...
function readHavliandAgentManifestFile(packageJsonPath: string): HavliandAgentManifest | null {
	try {
		const content = readFileSync(packageJsonPath, "utf-8");
		const pkg = JSON.parse(content) as { havliand_agent?: HavliandAgentManifest };
		return pkg.havliand_agent ?? null;
	} catch {
		return null;
	}
}
...
function resolveExtensionEntries(dir: string): string[] | null {
	...
	if (existsSync(packageJsonPath)) {
		const manifest = readHavliandAgentManifestFile(packageJsonPath);
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
	if (resourceType === "skills") {
		return collectSkillEntries(dir, "havliand_agent");
	}
...
export class DefaultPackageManager implements PackageManager {
	...
	private getNpmInstallArgs(specs: string[], installRoot: string): string[] {
		...
		// Extension packages run inside havliand_agent and resolve HavliandAgent APIs through loader aliases/virtual modules.
		// Disable peer dependency resolution for managed installs (npm's --legacy-peer-deps, and
		// equivalent bun/pnpm settings) so package managers do not install or solve host-provided
		// @havliand_agent/* peers. Stale auto-installed HavliandAgent peers can otherwise block updates.
	...
	private ensureNpmProject(installRoot: string): void {
		...
		if (!existsSync(packageJsonPath)) {
			const pkgJson = { name: "havliand_agent-extensions", private: true };
	...
	private collectPackageResources(
		...
		const manifest = this.readHavliandAgentManifest(packageRoot);
		if (manifest) {
			for (const resourceType of RESOURCE_TYPES) {
				const entries = manifest[resourceType as keyof HavliandAgentManifest];
	...
	private readHavliandAgentManifest(packageRoot: string): HavliandAgentManifest | null {
	...
	private addAutoDiscoveredResources(
		...
		if (projectTrusted) {
			// Project extensions from .havliand_agent/
			addResources(
				"extensions",
			...
			// Project skills from .havliand_agent/
			addResources(
				"skills",
				collectAutoSkillEntries(projectDirs.skills, "havliand_agent"),
				projectMetadata,
		...
		// User extensions from ~/.havliand_agent/agent/
		addResources(
			"extensions",
		...
		// User skills from ~/.havliand_agent/agent/
		addResources(
			"skills",
			collectAutoSkillEntries(userDirs.skills, "havliand_agent"),
			userMetadata,
