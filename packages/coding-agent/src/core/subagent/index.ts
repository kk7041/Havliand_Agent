 ...
 *
 * Spawns a separate `havliand_agent` process for each subagent invocation,
...
import type { AgentToolResult } from "@havliand_agent/agent-core";
import type { Message } from "@havliand_agent/ai";
...
async function writePromptToTempFile(agentName: string, prompt: string): Promise<{ dir: string; filePath: string }> {
	const tmpDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "havliand_agent-subagent-"));
...
function getPiInvocation(args: string[]): { command: string; args: string[] } {
	...
	return { command: "havliand_agent", args };
}
	...
	try {
		...
		const exitCode = await new Promise<number>((resolve) => {
			...
			const processLine = (line: string) => {
				if (!line.trim()) return;
				let event: SubagentProcessEvent;
				try {
					event = JSON.parse(line) as SubagentProcessEvent;
				} catch {
					return;
				}
...
export default function (havliandAgent: ExtensionAPI) {
	havliandAgent.registerTool(createSubagentToolDefinition());
}
