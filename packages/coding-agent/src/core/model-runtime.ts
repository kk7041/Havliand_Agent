...
import * as builtinProviderCatalog from "@havliand_agent/ai/providers/all";
...
/** Configured havliand_agent-ai Models collection used by coding-agent and SDK consumers. */
export class ModelRuntime implements Models {
	...
	static async create(options: CreateModelRuntimeOptions = {}): Promise<ModelRuntime> {
		...
		const runtime = new ModelRuntime(
			...
			modelsStore,
			providers,
			options.allowModelNetwork ?? process.env.HAVLIAND_AGENT_OFFLINE === undefined,
		);
	...
	async refresh(options: ModelsRefreshOptions = {}): Promise<ModelsRefreshResult> {
		...
		// Published havliand_agent-ai builds before ModelsStore returned void and accepted a provider ID.
