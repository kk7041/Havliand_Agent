import { type AgentMessage, uuidv7 } from "@havliand_agent/agent-core";
...
export interface CompactionEntry<T = unknown> extends SessionEntryBase {
	...
	details?: T;
	/** True if generated by an extension, undefined/false if HavliandAgent-generated (backward compatible) */
	fromHook?: boolean;
}
...
export interface BranchSummaryEntry<T = unknown> extends SessionEntryBase {
	...
	details?: T;
	/** True if generated by an extension, false if HavliandAgent-generated */
	fromHook?: boolean;
}
 ...
 * Encodes cwd into a safe directory name under ~/.havliand_agent/agent/sessions/.
 */
...
export class SessionManager {
	...
	setSessionFile(sessionFile: string): void {
		...
		if (existsSync(this.sessionFile)) {
			...
			// non-empty but did not parse as a HavliandAgent session, fail without modifying it.
			if (this.fileEntries.length === 0) {
				const explicitPath = this.sessionFile;
				if (statSync(explicitPath).size > 0) {
					throw new Error(`Session file is not a valid HavliandAgent session: ${explicitPath}`);
				}
				this.newSession();
	 ...
	 * @param sessionDir Optional session directory. If omitted, uses default (~/.havliand_agent/agent/sessions/<encoded-cwd>/).
	 */
