...
import type {
	...
	ThinkingLevel,
	ToolExecutionMode,
} from "@havliand_agent/agent-core";
import type {
	Api,
...
export interface ExtensionUIContext {
	 ...
	 * extend `CustomEditor` from `@havliand_agent/coding-agent` and call
	 * `super.handleInput(data)` for keys you don't handle.
	 *
	 * @example
	 * ```ts
	 * import { CustomEditor } from "@havliand_agent/coding-agent";
	 *
...
export interface ExtensionContext {
	...
	/** Gracefully shutdown havliand_agent and exit. Available in all contexts. */
	shutdown(): void;
...
export interface BeforeAgentStartEvent {
	...
	systemPrompt: string;
	/** Structured options used to build the system prompt. Extensions can inspect this to understand what havliand_agent loaded without re-discovering resources. */
...
/** Configuration for registering a provider via havliand_agent.registerProvider(). */
...
export type ExtensionFactory = (havliand_agent: ExtensionAPI) => void | Promise<void>;
...
/**
 * Action implementations for havliand_agent extension API methods.
