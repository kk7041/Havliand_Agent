import type { Api, Model, ProviderHeaders } from "@havliand_agent/ai";
...
function getDefaultAttributionHeaders(
	...
	if (isOpenRouterModel(model)) {
		return {
			"HTTP-Referer": "https://havliand_agent.dev",
			"X-OpenRouter-Title": "havliand_agent",
	...
	if (isNvidiaNimModel(model)) {
		return {
			"X-BILLING-INVOKE-ORIGIN": "HavliandAgent",
		};
	}

	if (isCloudflareModel(model)) {
		return {
			"User-Agent": "havliand_agent-coding-agent",
		};
	}
...
function getSessionHeaders(model: Model<Api>, sessionId: string | undefined): Record<string, string> | undefined {
	...
	return { "x-opencode-session": sessionId, "x-opencode-client": "havliand_agent" };
}
