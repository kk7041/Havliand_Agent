import { Markdown, type MarkdownTheme } from "@havliand_agent/tui";
...
import { getLatestHavliandAgentRelease, isNewerPackageVersion } from "./utils/version-check.ts";
import {
...
function getPackageCommandUsage(command: PackageCommand): string {
	switch (command) {
		...
		case "update":
			return `${APP_NAME} update [source|self|havliand_agent] [--self|--extensions|--all] [--extension <source>] [--approve|--no-approve] [--force]`;
		case "list":
...
Update havliand_agent and installed packages.

Options:
  --self                  Update havliand_agent only (default when no target is given)
  --extensions            Update installed packages only
  --all                   Update havliand_agent and installed packages
  --extension <source>    Update one package only
  -a, --approve           Trust project-local files for this command
  -na, --no-approve       Ignore project-local files for this command
  --force                 Reinstall havliand_agent even if the current version is latest

Short forms:
  ${APP_NAME} update                Update havliand_agent only
  ${APP_NAME} update --all          Update havliand_agent and all extensions
  ${APP_NAME} update <source>       Update one package
  ${APP_NAME} update havliand_agent       Update havliand_agent only (self works as alias)
`);
			return;
...
function parsePackageCommand(args: string[]): PackageCommandOptions | undefined {
	...
	if (command === "update") {
		...
		if (extensionFlagSource) {
			...
			const sourceIsSelf = source === "self" || source === "havliand_agent";
			if (sourceIsSelf) {
...
function printSelfUpdateUnavailable(
	...
	if (entrypoint) {
		console.error("");
		console.error(`Location of HavliandAgent executable: ${entrypoint}`);
	}
}
...
async function getSelfUpdatePlan(force: boolean): Promise<SelfUpdatePlan> {
	let latestRelease: Awaited<ReturnType<typeof getLatestHavliandAgentRelease>>;
	try {
		latestRelease = await getLatestHavliandAgentRelease(VERSION);
	} catch (error: unknown) {
