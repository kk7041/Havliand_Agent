 ...
 *
 * Usage:
 *   havliand_agent -e ./ssh.ts --ssh user@host
 *   havliand_agent -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (havliand_agent: ExtensionAPI) {
	havliand_agent.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	havliand_agent.on("session_start", async (_event, ctx) => {
		...
		const arg = havliand_agent.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
	...
	havliand_agent.on("user_bash", (_event) => {
		const ssh = getSsh();
