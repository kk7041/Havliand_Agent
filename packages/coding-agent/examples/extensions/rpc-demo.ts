...
export default function (havliand_agent: ExtensionAPI) {
	...
	havliand_agent.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "havliand_agent RPC Demo (new session)" : "havliand_agent RPC Demo");
	...
	havliand_agent.registerCommand("rpc-input", {
	...
	havliand_agent.registerCommand("rpc-editor", {
	...
	havliand_agent.registerCommand("rpc-prefill", {
