...
import type { ExtensionAPI, ExtensionContext, Theme, ToolExecutionMode } from "@havliand_agent/coding-agent";
import { type Component, matchesKey, Text, truncateToWidth, visibleWidth } from "@havliand_agent/tui";
...
export default function (havliand_agent: ExtensionAPI) {
	havliand_agent.on("session_start", async (_event, ctx) => reconstructState(ctx));
	havliand_agent.on("session_tree", async (_event, ctx) => reconstructState(ctx));
	...
	// -----------------------------------------------------------------------
	havliand_agent.registerMessageRenderer(MOVE_MESSAGE_TYPE, (message, { expanded }, theme) => {
	...
	// -----------------------------------------------------------------------
	havliand_agent.registerMessageRenderer(GAME_OVER_MESSAGE_TYPE, (message, _options, theme) => {
		const details = message.details as BoardDetails | undefined;
...
`;
	...
	// /tic-tac-toe command
	// -----------------------------------------------------------------------
	havliand_agent.registerCommand("tic-tac-toe", {
		...
		handler: async (_args, ctx) => {
			...
			gameActive = true;
			havliand_agent.setSessionName("Tic-Tac-Toe");
			...
			await ctx.ui.custom<void>((tui, _theme, _kb, done) => {
				component = new TicTacToeComponent(
					...
					(row, col) => {
						...
						havliand_agent.appendEntry(SAVE_TYPE, getBoardDetails());
