...
async function checkDirtyRepo(
	havliand_agent: ExtensionAPI,
	ctx: ExtensionContext,
	action: string,
): Promise<{ cancel: boolean } | undefined> {
	// Check for uncommitted changes
	const { stdout, code } = await havliand_agent.exec("git", ["status", "--porcelain"]);
...
export default function (havliand_agent: ExtensionAPI) {
	havliand_agent.on("session_before_switch", async (event, ctx) => {
		...
		return checkDirtyRepo(havliand_agent, ctx, action);
	});

	havliand_agent.on("session_before_fork", async (_event, ctx) => {
		return checkDirtyRepo(havliand_agent, ctx, "fork");
	});
}
