 ...
 *
 * Demonstrates havliand_agent.sendUserMessage() for sending user messages from extensions.
 * Unlike havliand_agent.sendMessage() which sends custom messages, sendUserMessage() sends
...
export default function (havliand_agent: ExtensionAPI) {
	...
	havliand_agent.registerCommand("ask", {
		...
		handler: async (args, ctx) => {
			...
			havliand_agent.sendUserMessage(args);
		},
	});

	// Command that steers the agent mid-conversation
	havliand_agent.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				havliand_agent.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	havliand_agent.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				havliand_agent.sendUserMessage(args, { deliverAs: "followUp" });
	...
	havliand_agent.registerCommand("askwith", {
