...
export default function (havliand_agent: ExtensionAPI) {
	havliand_agent.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await havliand_agent.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[havliand_agent] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await havliand_agent.exec("git", ["add", "-A"]);
		const { code: commitCode } = await havliand_agent.exec("git", ["commit", "-m", commitMessage]);
