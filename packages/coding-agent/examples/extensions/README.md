...
Example extensions for havliand_agent-coding-agent.

## Usage

```bash
# Load an extension with --extension flag
havliand_agent --extension examples/extensions/permission-gate.ts

# Or copy to extensions directory for auto-discovery
cp permission-gate.ts ~/.havliand_agent/agent/extensions/
```
...
| `send-user-message.ts` | Demonstrates `havliand_agent.sendUserMessage()` for sending user messages from extensions |
...
| `im-webhook/` | HTTP webhook listener that injects external IM messages into the active session, including VPS, systemd, nginx, TLS, and logging guidance |
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs havliand_agent theme with macOS dark/light mode |
...
| `event-bus.ts` | Inter-extension communication via `havliand_agent.events` |
...
| `custom-provider-gitlab-duo/` | GitLab Duo provider using havliand_agent-ai's built-in Anthropic/OpenAI streaming via proxy |
