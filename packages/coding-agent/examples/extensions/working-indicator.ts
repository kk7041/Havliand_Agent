 ...
 * working indicator shown while havliand_agent is streaming a response.
 *
 * Usage:
 *   havliand_agent --extension examples/extensions/working-indicator.ts
 *
 * Commands:
 ...
 *   /working-indicator reset     Restore havliand_agent's default spinner
 */

import type { ExtensionAPI, ExtensionContext, WorkingIndicatorOptions } from "@havliand_agent/coding-agent";
...
function describeMode(mode: WorkingIndicatorMode): string {
	switch (mode) {
		...
		case "default":
			return "havliand_agent default spinner";
	}
}
...
export default function (havliand_agent: ExtensionAPI) {
	...
	havliand_agent.registerCommand("working-indicator", {
