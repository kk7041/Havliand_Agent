...
import type { Component, EditorTheme, TUI } from "@havliand_agent/tui";
import { truncateToWidth, visibleWidth } from "@havliand_agent/tui";
...
export default function (havliand_agent: ExtensionAPI) {
	...
	havliand_agent.on("session_start", (_event, ctx) => {
		...
		const refreshBranch = async () => {
			const result = await havliand_agent
		...
		class BorderStatusEditor extends CustomEditor {
			...
			render(width: number): string[] {
				...
				const thinking = havliand_agent.getThinkingLevel();
