 ...
 * - ~/.havliand_agent/agent/presets.json (global)
 * - <cwd>/.havliand_agent/presets.json (project-local)
 *
 ...
 *
 * Usage:
 * - `havliand_agent --preset plan` - start with plan preset
...
import type { Api, Model } from "@havliand_agent/ai";
import type { ExtensionAPI, ExtensionContext } from "@havliand_agent/coding-agent";
import { CONFIG_DIR_NAME, DynamicBorder, getAgentDir } from "@havliand_agent/coding-agent";
import { Container, Key, type SelectItem, SelectList, Text } from "@havliand_agent/tui";
...
export default function presetExtension(havliand_agent: ExtensionAPI) {
	...
	havliand_agent.registerFlag("preset", {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			originalState = {
				model: ctx.model,
				thinkingLevel: havliand_agent.getThinkingLevel(),
				tools: havliand_agent.getActiveTools(),
			};
		}
		...
		if (preset.thinkingLevel) {
			havliand_agent.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = havliand_agent.getAllTools().map((t) => t.name);
			...
			if (validTools.length > 0) {
				havliand_agent.setActiveTools(validTools);
			}
		}
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await havliand_agent.setModel(originalState.model);
				}
				havliand_agent.setThinkingLevel(originalState.thinkingLevel);
				havliand_agent.setActiveTools(originalState.tools);
			} else {
				havliand_agent.setActiveTools(["read", "bash", "edit", "write"]);
			}
	...
	havliand_agent.registerShortcut(Key.ctrlShift("u"), {
	...
	havliand_agent.registerCommand("preset", {
	...
	havliand_agent.on("before_agent_start", async (event) => {
	...
	havliand_agent.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = havliand_agent.getFlag("preset");
	...
	havliand_agent.on("turn_start", async () => {
		if (activePresetName) {
			havliand_agent.appendEntry("preset-state", { name: activePresetName });
		}
	});
}
