 ...
 *   havliand_agent -e ./examples/extensions/git-merge-and-resolve.ts
 */
...
async function findConflicts(havliand_agent: ExtensionAPI, cwd: string): Promise<ConflictBlock[]> {
	const { stdout, code } = await havliand_agent.exec("git", ["diff", "--name-only", "--diff-filter=U"]);
...
export default function (havliand_agent: ExtensionAPI) {
	havliand_agent.on("agent_end", async (_event, ctx) => {
		const { code: revParseCode } = await havliand_agent.exec("git", ["rev-parse", "--git-dir"]);
		if (revParseCode !== 0) return;

		let ref = "MERGE_HEAD";

		// If not already in a merge, attempt one
		const { code: mergeHeadCode } = await havliand_agent.exec("git", ["rev-parse", "MERGE_HEAD"]);
		if (mergeHeadCode !== 0) {
			// Only attempt a new merge if the working tree is clean
			const { stdout: status } = await havliand_agent.exec("git", ["status", "--porcelain"]);
			if (status.trim()) return;

			const { stdout: upstream, code: upstreamCode } = await havliand_agent.exec("git", [
				"rev-parse",
				"--abbrev-ref",
				"--symbolic-full-name",
				"@{u}",
			...
			const { code: fetchCode, stderr: fetchErr } = await havliand_agent.exec("git", ["fetch", remote]);
			if (fetchCode !== 0) {
				ctx.ui.notify(`git-merge-and-resolve: fetch failed: ${fetchErr.trim()}`, "warning");
				return;
			}

			const { code: mergeCode } = await havliand_agent.exec("git", ["merge", "--no-ff", ref]);
			if (mergeCode === 0) return;
		}

		// Either we just merged with conflicts, or we were already in an unfinished merge
		const conflicts = await findConflicts(havliand_agent, ctx.cwd);
		if (conflicts.length === 0) return;

		havliand_agent.sendUserMessage(formatConflicts(ref, conflicts), { deliverAs: "followUp" });
	});
}
