...
import { isKeyRelease, Key, matchesKey, visibleWidth } from "@havliand_agent/tui";
...
export default function (havliand_agent: ExtensionAPI) {
	havliand_agent.registerCommand("invaders", {
		...
		handler: async (_args, ctx) => {
			...
			await ctx.ui.custom((tui, _theme, _kb, done) => {
				return new SpaceInvadersComponent(
					...
					() => done(undefined),
					(state) => {
						havliand_agent.appendEntry(INVADERS_SAVE_TYPE, state);
					},
					savedState,
				);
			});
