...
async function resolveGitHubRepo(havliand_agent: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await havliand_agent.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (havliand_agent: ExtensionAPI): void {
	havliand_agent.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(havliand_agent, ctx.cwd);
		if (!resolvedRepo.ok) {
		...
		const getIssues = async (): Promise<GitHubIssue[] | undefined> => {
			issuesPromise ||= (async () => {
				const result = await havliand_agent.exec(
					"gh",
					[
						"issue",
						"list",
