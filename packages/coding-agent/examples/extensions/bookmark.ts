...
export default function (havliand_agent: ExtensionAPI) {
	havliand_agent.registerCommand("bookmark", {
		...
		handler: async (args, ctx) => {
			...
			for (let i = entries.length - 1; i >= 0; i--) {
				...
				if (entry.type === "message" && entry.message.role === "assistant") {
					havliand_agent.setLabel(entry.id, label);
	...
	// Remove bookmark
	havliand_agent.registerCommand("unbookmark", {
		...
		handler: async (_args, ctx) => {
			...
			for (let i = entries.length - 1; i >= 0; i--) {
				...
				if (label) {
					havliand_agent.setLabel(entry.id, undefined);
