...
import type { ExtensionAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "@havliand_agent/coding-agent";

export default function (havliand_agent: ExtensionAPI) {
	havliand_agent.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;
