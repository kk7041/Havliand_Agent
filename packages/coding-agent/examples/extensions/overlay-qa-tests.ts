 ...
 *
 * Usage: havliand_agent --extension ./examples/extensions/overlay-qa-tests.ts
 *
 * Commands:
...
import type { ExtensionAPI, ExtensionCommandContext, Theme } from "@havliand_agent/coding-agent";
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "@havliand_agent/tui";
import { Input, matchesKey, truncateToWidth, visibleWidth } from "@havliand_agent/tui";
...
export default function (havliand_agent: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates (like havliand_agent-doom would need)
	havliand_agent.registerCommand("overlay-animation", {
	...
	havliand_agent.registerCommand("overlay-anchors", {
	...
	havliand_agent.registerCommand("overlay-margins", {
	...
	havliand_agent.registerCommand("overlay-stack", {
	...
	havliand_agent.registerCommand("overlay-overflow", {
	...
	havliand_agent.registerCommand("overlay-edge", {
	...
	havliand_agent.registerCommand("overlay-percent", {
	...
	// Test maxHeight
	havliand_agent.registerCommand("overlay-maxheight", {
	...
	havliand_agent.registerCommand("overlay-sidepanel", {
	...
	havliand_agent.registerCommand("overlay-toggle", {
	...
	havliand_agent.registerCommand("overlay-passive", {
	...
	havliand_agent.registerCommand("overlay-focus", {
	...
	havliand_agent.registerCommand("overlay-streaming", {
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			for i in $(seq 1 100); do
				...
				DIR="/Users/nicobailon/Documents/development/havliand_agent-mono/packages/coding-agent/src/modes/interactive"
				...
				if [ $((i % 5)) -eq 0 ]; then
					echo -e "  \\033[32m✓ Successfully processed \${i} files in /Users/nicobailon/Documents/development/havliand_agent-mono\\033[0m"
				fi
				if [ $((i % 7)) -eq 0 ]; then
...
// Animation demo - proves overlays can handle real-time updates like havliand_agent-doom
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " (havliand_agent-doom uses same approach)")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));
