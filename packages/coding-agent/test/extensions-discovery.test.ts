...
describe("extensions discovery", () => {
	...
	it("loads the coding-agent entrypoint without rewriting havliand_agent-ai provider subpaths", async () => {
		fs.writeFileSync(
			path.join(extensionsDir, "coding-agent-import.ts"),
			`
				import { getAgentDir } from "@havliand_agent/coding-agent";
				void getAgentDir;
				export default function(pi) {
					pi.registerCommand("test", { handler: async () => {} });
				}
	...
	it("keeps the type-only havliand_agent-ai OAuth compatibility barrel resolvable", async () => {
		fs.writeFileSync(
			path.join(extensionsDir, "oauth-import.ts"),
			`
				import * as oauth from "@havliand_agent/ai/oauth";
				void oauth;
				export default function(pi) {
					pi.registerCommand("test", { handler: async () => {} });
				}
