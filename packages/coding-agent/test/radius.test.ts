...
import { InMemoryModelsStore } from "@havliand_agent/ai";
...
beforeEach(() => {
	tempDir = join(tmpdir(), `havliand_agent-test-radius-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe("Radius provider", () => {
	it("restores the legacy credential catalog without network access", async () => {
		...
		expect(model).toMatchObject({ api: "havliand_agent-messages", baseUrl: "https://radius.example.com/v1" });
	...
	it("does not fetch or expose Radius models without configured auth", async () => {
		...
		expect(fetchSpy.mock.calls.some(([url]) => String(url).includes("radius.havliand_agent.dev/v1/config"))).toBe(
			false,
		);
	});
