...
describe("SettingsManager", () => {
	...
	describe("error tracking", () => {
		it("should collect and clear load errors via drainErrors", () => {
			...
			const projectSettingsPath = join(projectDir, ".havliand_agent", "settings.json");
	...
	describe("project trust", () => {
		it("should skip project settings when project is not trusted", () => {
			...
			writeFileSync(join(projectDir, ".havliand_agent", "settings.json"), JSON.stringify({ theme: "project" }));
	...
	describe("project settings directory creation", () => {
		it("should not create .havliand_agent folder when only reading project settings", () => {
			// Create agent dir with global settings, but NO .havliand_agent folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));

			// Delete the .havliand_agent folder that beforeEach created
			rmSync(join(projectDir, ".havliand_agent"), { recursive: true });

			// Create SettingsManager (reads both global and project settings)
			const manager = SettingsManager.create(projectDir, agentDir);

			// .havliand_agent folder should NOT have been created just from reading
			expect(existsSync(join(projectDir, ".havliand_agent"))).toBe(false);

			// Settings should still be loaded from global
			expect(manager.getTheme()).toBe("dark");
		});

		it("should create .havliand_agent folder when writing project settings", async () => {
			// Create agent dir with global settings, but NO .havliand_agent folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));
			...
			// .havliand_agent folder should NOT exist yet
			expect(existsSync(join(projectDir, ".havliand_agent"))).toBe(false);
			...
			// Now .havliand_agent folder should exist
			expect(existsSync(join(projectDir, ".havliand_agent"))).toBe(true);

			// And settings file should be created
			expect(existsSync(join(projectDir, ".havliand_agent", "settings.json"))).toBe(true);
		});
	});
	...
	describe("httpIdleTimeoutMs", () => {
		...
		it("should use merged global and project settings", () => {
			...
			writeFileSync(join(projectDir, ".havliand_agent", "settings.json"), JSON.stringify({ httpIdleTimeoutMs: 0 }));
