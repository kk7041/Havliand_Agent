...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".havliand_agent", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".havliand_agent", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".havliand_agent", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".havliand_agent"), { recursive: true });
			writeFileSync(join(cwd, ".havliand_agent", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".havliand_agent"), { recursive: true, force: true });
