...
describe("package commands", () => {
	...
	beforeEach(() => {
		...
		originalPiPackageDir = process.env.HAVLIAND_AGENT_PACKAGE_DIR;
	...
	it("should persist global relative local package paths relative to settings.json", async () => {
		...
		const stored = settings.packages?.[0] ?? "-agent";
	...
	it("skips untrusted project package settings", async () => {
		mkdirSync(join(projectDir, ".havliand_agent"), { recursive: true });
		writeFileSync(
			join(projectDir, ".havliand_agent", "settings.json"),
	...
	it("blocks local package changes when project is untrusted", async () => {
		mkdirSync(join(projectDir, ".havliand_agent"), { recursive: true });
		writeFileSync(join(projectDir, ".havliand_agent", "settings.json"), "{}");
	...
	it("allows local package install to initialize fresh project settings", async () => {
		...
		const settingsPath = join(projectDir, ".havliand_agent", "settings.json");
		const settings = JSON.parse(readFileSync(settingsPath, "utf-8")) as { packages?: string[] };
		expect(settings.packages?.length).toBe(1);
		const stored = settings.packages?.[0] ?? "-agent";
		expect(realpathSync(join(projectDir, ".havliand_agent", stored))).toBe(realpathSync(packageDir));
		expect(process.exitCode).toBeUndefined();
	});
	...
	it("shows a friendly error for unknown install options", async () => {
		...
		try {
			...
			expect(stderr).toContain(
				'Use "havliand_agent --help" or "pi install <source> [-l] [--approve|--no-approve]".',
			);
			expect(process.exitCode).toBe(1);
		} finally {
			errorSpy.mockRestore();
	...
	it("shows a friendly error for missing install source", async () => {
		...
		try {
			...
			expect(stderr).toContain("Usage: havliand_agent install <source> [-l]");
...
`,
		...
		process.env.HAVLIAND_AGENT_PACKAGE_DIR = selfPackageDir;
		...
		try {
			...
			expect(stdout).toContain(`Updated havliand_agent from ${VERSION} to ${VERSION}`);
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
	...
	it("uses the current package name when the update check omits packageName", async () => {
		const globalPrefix = join(tempDir, "global-prefix");
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@havliand_agent", "coding-agent");
		const fakeNpmPath = join(tempDir, "fake-npm.cjs");
		const recordPath = join(tempDir, "self-update.json");
		mkdirSync(selfPackageDir, { recursive: true });
		writeFileSync(
...
`,
		...
		try {
			...
			expect(recordedArgs).not.toContain(PACKAGE_NAME);
			expect(stdout).toContain(`Updated havliand_agent from ${VERSION} to ${targetVersion}`);
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
...
`,
		...
		const activePackageName =
			PACKAGE_NAME === "@new-scope/havliand_agent" ? "@newer-scope/havliand_agent" : "@new-scope/havliand_agent";
		vi.stubGlobal(
			"fetch",
	...
	it("prints a pnpm metadata hint when self-update fails", async () => {
		...
		const selfPackageDir = join(globalRoot, "node_modules", "@earendil@havliand_agent-agent", "coding-agent");
		...
		process.env.PATH = `${fakeBinDir}${process.env.PATH ? `${delimiter}${process.env.PATH}` : "-agent"}`;
		process.env.HAVLIAND_AGENT_PACKAGE_DIR = selfPackageDir;
		Object.defineProperty(process, "execPath", {
		...
		try {
			...
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
			const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stdout).not.toContain("Updated havliand_agent");
			expect(stderr).toContain("exited with code 23");
			expect(stderr).toContain("If pnpm reports missing package versions");
			expect(stderr).toContain("Run `pnpm store prune` and retry `havliand_agent update --self`.");
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
...
`,
	...
	it("suggests the configured source when update input omits the npm prefix", async () => {
		const settingsPath = join(agentDir, "settings.json");
		writeFileSync(settingsPath, JSON.stringify({ packages: ["npm:havliand_agent-formatter"] }, null, 2));

		const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
		const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});

		try {
			await expect(main(["update", "havliand_agent-formatter"])).resolves.toBeUndefined();

			const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stderr).toContain("Did you mean npm:havliand_agent-formatter?");
			expect(stdout).not.toContain("Updated havliand_agent-formatter");
			expect(process.exitCode).toBe(1);

			const settings = JSON.parse(readFileSync(settingsPath, "utf-8")) as { packages?: string[] };
			expect(settings.packages).toContain("npm:havliand_agent-formatter");
		} finally {
			errorSpy.mockRestore();
			logSpy.mockRestore();
		}
