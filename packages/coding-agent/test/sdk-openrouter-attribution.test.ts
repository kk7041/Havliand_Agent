...
describe("createAgentSession provider attribution headers", () => {
	...
	beforeEach(() => {
		...
		originalTelemetryEnv = process.env.HAVLIAND_AGENT_TELEMETRY;
		delete process.env.HAVLIAND_AGENT_TELEMETRY;
	});
	...
	afterEach(() => {
		if (originalTelemetryEnv === undefined) {
			...
			process.env.HAVLIAND_AGENT_TELEMETRY = originalTelemetryEnv;
		}
	...
	it("adds default attribution headers for OpenRouter models", async () => {
		...
		expect(headers?.["HTTP-Referer"]).toBe("https://havliand_agent.dev");
		expect(headers?.["X-OpenRouter-Title"]).toBe("havliand_agent");
	...
	it("adds OpenCode session headers", async () => {
		...
		expect(headers?.["x-opencode-client"]).toBe("havliand_agent");
	});
