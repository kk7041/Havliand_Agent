...
import { Agent } from "@havliand_agent/agent-core";
import type { OAuthCredentials } from "@havliand_agent/ai";
import { getModel } from "@havliand_agent/ai/compat";
import { builtinProviders } from "@havliand_agent/ai/providers/all";
...
// ============================================================================
// OAuth API key resolution from ~/.havliand_agent/agent/auth.json
// ============================================================================

const AUTH_PATH = join(homedir(), ".havliand_agent", "agent", "auth.json");
...
/**
 * Resolve API key for a provider from ~/.havliand_agent/agent/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.havliand_agent/agent/auth.json
 */
...
/** Path to the real havliand_agent config directory */
export const HAVLIAND_AGENT_DIR = join(homedir(), ".havliand_agent", "agent");

/**
 * Get an AuthStorage instance backed by ~/.havliand_agent/agent/auth.json
...
export async function createTestSession(options: TestSessionOptions = {}): Promise<TestSessionContext> {
	const tempDir = join(tmpdir(), `havliand_agent-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
