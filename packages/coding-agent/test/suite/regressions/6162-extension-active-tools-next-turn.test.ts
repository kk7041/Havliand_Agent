import { fauxAssistantMessage, fauxToolCall } from "@havliand_agent/ai";
...
describe("extension active tools next-turn refresh", () => {
	it("applies havliand_agent.setActiveTools before the next provider request in the same run", async () => {
		const extensionFactories: ExtensionFactory[] = [
			(havliand_agent) => {
				havliand_agent.registerTool({
					...
					execute: async () => {
						havliand_agent.setActiveTools(["after_switch"]);
						return {
		...
		const extensionFactories: ExtensionFactory[] = [
			(havliand_agent) => {
				havliand_agent.registerTool({
					...
					parameters: Type.Object({}),
					execute: async () => {
						havliand_agent.setActiveTools([...havliand_agent.getActiveTools(), "after_load"]);
						return {
