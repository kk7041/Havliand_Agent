...
import type { AnthropicMessagesCompat, Api, Context, Model, OpenAICompletionsCompat } from "@havliand_agent/ai/compat";
import { getApiProvider, getSupportedThinkingLevels } from "@havliand_agent/ai/compat";
...
describe("ModelRegistry", () => {
	...
	beforeEach(() => {
		tempDir = join(
			tmpdir(),
			`havliand_agent-test-model-registry-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
