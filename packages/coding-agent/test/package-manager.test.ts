...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.HAVLIAND_AGENT_OFFLINE;
		delete process.env.HAVLIAND_AGENT_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.HAVLIAND_AGENT_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .havliand_agent skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .havliand_agent", async () => {
			const extDir = join(tempDir, ".havliand_agent", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".havliand_agent"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".havliand_agent", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".havliand_agent", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".havliand_agent", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".havliand_agent", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".havliand_agent", "prompts");
			mkdirSync(promptsDir, { recursive: true });
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .havliand_agent/agent skills", async () => {
		...
		it("should use the project .havliand_agent dir as baseDir for project .havliand_agent skills", async () => {
			const projectBaseDir = join(tempDir, ".havliand_agent");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".havliand_agent", "agent");
		...
		it("should dedupe user skill entries when ~/.havliand_agent/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .havliand_agent auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".havliand_agent\n");

			const skillDir = join(tempDir, ".havliand_agent", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.havliand_agent\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".havliand_agent", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
	...
	describe("source parsing", () => {
		...
		it("should parse package source types from docs examples", () => {
			...
			expect(parseNpm("npm:@scope/pkg@1.2.3").havliand_agentnned).toBe(true);
			expect(parseNpm("npm:@scope/pkg@^1.2.3").havliand_agentnned).toBe(false);
			expect(parseNpm("npm:pkg").havliand_agentnned).toBe(false);
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .havliand_agent settings base", () => {
			...
			const rel = relative(join(tempDir, ".havliand_agent"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("HTTPS git URL parsing (old behavior)", () => {
		it("should parse HTTPS GitHub URLs correctly", async () => {
			...
			expect(parsed.havliand_agentnned).toBe(false);
		});
		...
		it("should parse HTTPS URLs with ref", async () => {
			...
			expect(parsed.havliand_agentnned).toBe(true);
		});
	...
	describe("pattern filtering in package filters", () => {
		...
		it("should resolve autoload-disabled package entries as positive-only without a global package", async () => {
			...
			settingsManager.setProjectPackages([
				{
					source: relative(join(tempDir, ".havliand_agent"), pkgDir),
					autoload: false,
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("offline mode and network timeouts", () => {
		it("should update npm range packages using the configured spec", async () => {
			const installedPath = join(tempDir, ".havliand_agent", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@^1.0.0", "--prefix", join(tempDir, ".havliand_agent", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".havliand_agent", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".havliand_agent", "npm", "node_modules", "project-current");
			...
			expect(runCommandSpy).toHaveBeenNthCalledWith(
				...
				[
					...
					"--prefix",
					join(tempDir, ".havliand_agent", "npm"),
					"--legacy-peer-deps",
				],
				undefined,
			);
		...
		it("should skip installing missing package sources when offline", async () => {
			process.env.HAVLIAND_AGENT_OFFLINE = "1";
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".havliand_agent", "npm", "node_modules", "example");
