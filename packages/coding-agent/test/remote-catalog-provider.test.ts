import { createProvider, InMemoryModelsStore, type Model } from "@havliand_agent/ai";
...
describe("remote catalog provider", () => {
	it("parses keyed catalogs, sends version headers, and observes the refresh TTL", async () => {
		...
		expect(fetchSpy.mock.calls[0]?.[1]?.headers).toMatchObject({
			"User-Agent": expect.stringContaining(`havliand_agent/${VERSION}`),
		});
	});

	it("treats unimplemented HavliandAgent catalog routes as an unavailable overlay", async () => {
