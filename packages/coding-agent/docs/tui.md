> havliand_agent can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`@havliand_agent/tui`](https://github.com/kk7041/havliand_agent/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@havliand_agent/tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with `showHardwareCursor`, `setShowHardwareCursor(true)`, or `HAVLIAND_AGENT_HARDWARE_CURSOR=1`. The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { Container, type Focusable, Input } from "@havliand_agent/tui";
...
**In custom tools** via `havliand_agent.ui.custom()`:

```typescript
async execute(toolCallId, params, onUpdate, ctx, signal) {
  const handle = havliand_agent.ui.custom(myComponent);
  // ...
  handle.close();
}
```
...
Import from `@havliand_agent/tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@havliand_agent/tui";
```
...
```typescript
import { matchesKey, Key } from "@havliand_agent/tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@havliand_agent/tui";
...
```typescript
havliand_agent.registerCommand("pick", {
...
```typescript
import { getMarkdownTheme } from "@havliand_agent/coding-agent";
import { Markdown } from "@havliand_agent/tui";
...
Set `HAVLIAND_AGENT_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
HAVLIAND_AGENT_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@havliand_agent/tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@havliand_agent/coding-agent";
import { DynamicBorder } from "@havliand_agent/coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@havliand_agent/tui";
...
```typescript
import { BorderedLoader } from "@havliand_agent/coding-agent";

havliand_agent.registerCommand("fetch", {
  handler: async (_args, ctx) => {
    const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
...
For toggling multiple settings. Use `SettingsList` from `@havliand_agent/tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@havliand_agent/coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@havliand_agent/tui";

havliand_agent.registerCommand("settings", {
  handler: async (_args, ctx) => {
...
Customize the inline working indicator shown while havliand_agent is streaming a response.
...
// Restore havliand_agent's default spinner
ctx.ui.setWorkingIndicator();
```
...
import { matchesKey, truncateToWidth } from "@havliand_agent/tui";
