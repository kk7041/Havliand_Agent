> havliand_agent can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to havliand_agent's agent capabilities. Use it to embed havliand_agent in other applications, build custom interfaces, or integrate with automated workflows.
...
```bash
npm install @havliand_agent/coding-agent
```
...
```typescript
import { createAgentSession, SessionManager } from "@havliand_agent/coding-agent";
...
**Behavior:**
- **Extension commands** (e.g., `/mycommand`): Execute immediately, even during streaming. They manage their own LLM interaction via `havliand_agent.sendMessage()`.
...
The `Agent` class (from `@havliand_agent/agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.havliand_agent/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.havliand_agent/extensions/`)
- Project skills:
  - `.havliand_agent/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.havliand_agent/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.havliand_agent/agent/skills/`)
  - `~/.agents/skills/`
...
```typescript
import { getModel } from "@havliand_agent/ai";
import { ModelRuntime } from "@havliand_agent/coding-agent";
...
```typescript
import { InMemoryCredentialStore } from "@havliand_agent/ai";
import { createAgentSession, ModelRuntime } from "@havliand_agent/coding-agent";

// Default: uses ~/.havliand_agent/agent/auth.json and ~/.havliand_agent/agent/models.json
const modelRuntime = await ModelRuntime.create();
...
// Or inject any havliand_agent-ai CredentialStore
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@havliand_agent/coding-agent";
...
The `edit` tool returns `details.diff` for havliand_agent's TUI display and `details.patch` as a standard unified patch for SDK consumers.

```typescript
import { createAgentSession } from "@havliand_agent/coding-agent";
...
import { createAgentSession, defineTool } from "@havliand_agent/coding-agent";
...
Use `defineTool()` for standalone definitions and arrays like `customTools: [myTool]`. Inline `havliand_agent.registerTool({ ... })` already infers parameter types correctly.

Custom tools passed via `customTools` are combined with extension-registered tools. Extensions loaded by the ResourceLoader can also register tools via `havliand_agent.registerTool()`.
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.havliand_agent/agent/extensions/`, `.havliand_agent/extensions/`, and settings.json extension sources.
...
```typescript
import type { InlineExtension } from "@havliand_agent/coding-agent";

const myProvider: InlineExtension = {
  name: "my-provider",
  factory: (havliand_agent) => {
    havliand_agent.on("agent_start", () => {
...
**Event Bus:** Extensions can communicate via `havliand_agent.events`. Pass a shared `eventBus` to `DefaultResourceLoader` if you need to emit or listen from outside:

```typescript
import { createEventBus, DefaultResourceLoader } from "@havliand_agent/coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@havliand_agent/coding-agent";
...
1. Global: `~/.havliand_agent/agent/settings.json`
2. Project: `<cwd>/.havliand_agent/settings.json`
...
```bash
havliand_agent --mode rpc --no-session
```
...
// Auth and Models
ModelRuntime // implements havliand_agent-ai Models and owns credential storage
