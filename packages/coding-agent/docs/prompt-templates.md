> havliand_agent can create prompt templates. Ask it to build one for your workflow.
...
havliand_agent loads prompt templates from:

- Global: `~/.havliand_agent/agent/prompts/*.md`
- Project: `.havliand_agent/prompts/*.md` (only after the project is trusted)
- Packages: `prompts/` directories or `havliand_agent.prompts` entries in `package.json`
