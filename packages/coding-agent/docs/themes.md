> havliand_agent can create themes. Ask it to build one for your setup.
...
havliand_agent loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.havliand_agent/agent/themes/*.json`
- Project: `.havliand_agent/themes/*.json` (only after the project is trusted)
- Packages: `themes/` directories or `havliand_agent.themes` entries in `package.json`
...
On first run, havliand_agent detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.havliand_agent/agent/themes
vim ~/.havliand_agent/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/kk7041/havliand_agent/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, havliand_agent reloads it automatically for immediate visual feedback.
...
havliand_agent uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, havliand_agent falls back to the nearest approximation.
