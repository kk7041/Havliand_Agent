...
Extensions can register custom model providers via `havliand_agent.registerProvider()`. This enables:
...
export default function (havliand_agent: ExtensionAPI) {
  ...
  havliand_agent.registerProvider("my-provider", {
    name: "My Provider",
...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. havliand_agent waits for the factory before startup continues, so the provider is available during interactive startup and to `havliand_agent --list-models`.
...
havliand_agent.registerProvider("openai", {
  headers: {
...
havliand_agent.registerProvider("google", {
...
```typescript
havliand_agent.registerProvider("my-llm", {
...
Use `havliand_agent.unregisterProvider(name)` to remove a provider that was previously registered via `havliand_agent.registerProvider(name, ...)`:
...
// Later, remove it
havliand_agent.unregisterProvider("my-llm");
```
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map havliand_agent levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
```typescript
havliand_agent.registerProvider("custom-api", {
  baseUrl: "https://api.example.com",
  apiKey: "$MY_API_KEY",
...
```typescript
import type { OAuthCredentials, OAuthLoginCallbacks } from "@havliand_agent/ai";
...
Credentials are persisted in `~/.havliand_agent/agent/auth.json`:
...
- [anthropic.ts](https://github.com/kk7041/havliand_agent/blob/main/packages/ai/src/providers/anthropic.ts) - Anthropic Messages API
- [mistral.ts](https://github.com/kk7041/havliand_agent/blob/main/packages/ai/src/providers/mistral.ts) - Mistral Conversations API
- [openai-completions.ts](https://github.com/kk7041/havliand_agent/blob/main/packages/ai/src/providers/openai-completions.ts) - OpenAI Chat Completions
- [openai-responses.ts](https://github.com/kk7041/havliand_agent/blob/main/packages/ai/src/providers/openai-responses.ts) - OpenAI Responses API
- [google.ts](https://github.com/kk7041/havliand_agent/blob/main/packages/ai/src/providers/google.ts) - Google Generative AI
- [amazon-bedrock.ts](https://github.com/kk7041/havliand_agent/blob/main/packages/ai/src/providers/amazon-bedrock.ts) - AWS Bedrock
...
When a request exceeds the model's context window, havliand_agent can recover automatically by compacting the conversation and retrying. This recovery only kicks in if havliand_agent recognizes the failure as an overflow.

Detection runs on the finalized assistant message:

- `stopReason === "error"`
- `errorMessage` matches one of havliand_agent's known overflow patterns (see [`packages/ai/src/utils/overflow.ts`](https://github.com/kk7041/havliand_agent/blob/main/packages/ai/src/utils/overflow.ts))

If your provider returns overflow errors with a message havliand_agent does not recognize, normalize the error from the same extension that registers the provider. Use a `message_end` handler to rewrite the assistant message so its `errorMessage` starts with a phrase havliand_agent recognizes. The generic fallback `context_length_exceeded` is the safest choice.
...
export default function (havliand_agent: ExtensionAPI) {
  ...
  havliand_agent.on("message_end", (event, ctx) => {
...
`message_end` runs before havliand_agent tracks the assistant message for auto-compaction, so the rewritten `errorMessage` is what havliand_agent checks. With this in place, havliand_agent will:
...
- Match a provider-specific pattern, not havliand_agent's generic overflow patterns. Rewriting rate-limit or throttling errors (`rate limit`, `too many requests`) would falsely trigger compaction instead of havliand_agent's normal retry-with-backoff path.
...
Test your provider against the same test suites used by built-in providers. Copy and adapt these test files from [packages/ai/test/](https://github.com/kk7041/havliand_agent/tree/main/packages/ai/test):
...
interface ProviderModelConfig {
  ...
  /** Maps havliand_agent thinking levels to provider/model-specific values; null marks a level unsupported. */
