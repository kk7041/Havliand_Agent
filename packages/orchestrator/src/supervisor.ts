...
export class OrchestratorSupervisor {
	...
	private updateRecord(live: LiveInstance, updates: Partial<InstanceRecord>): void {
		...
		if (updates.radiusHavliandAgentId !== undefined) {
			live.resources.radiusHavliandAgentId = updates.radiusHavliandAgentId;
		}
	...
	private async handleUnexpectedRpcExit(live: LiveInstance, _error?: Error): Promise<void> {
		...
		if (live.resources.radiusHavliandAgentId) {
			try {
				await radiusPresence.disconnectHavliandAgent(live.record);
				this.updateRecord(live, { radiusHavliandAgentId: undefined });
			} catch (error) {
				console.error(`Failed to disconnect Radius HavliandAgent ${live.record.id}: ${String(error)}`);
			}
		}
	...
	private async cleanupAcquiredResources(live: LiveInstance): Promise<void> {
		...
		if (live.resources.radiusHavliandAgentId) {
			await radiusPresence.disconnectHavliandAgent(live.record);
			live.resources.radiusHavliandAgentId = undefined;
			live.record = {
				...live.record,
				radiusHavliandAgentId: undefined,
				lastSeenAt: new Date().toISOString(),
			};
		}
	...
	updateInstance(instance: InstanceRecord): void {
		...
		if (live) {
			...
			live.resources.radiusHavliandAgentId = instance.radiusHavliandAgentId;
	...
	async recoverAfterRestart(): Promise<void> {
		...
		for (const instance of instances) {
			await radiusPresence.disconnectHavliandAgent(instance);
		}
	...
	async spawnInstance(options: { cwd: string; label?: string }): Promise<InstanceRecord> {
		...
		try {
			...
			await this.syncInstanceRecord(live);
			const registeredRecord = await radiusPresence.registerHavliandAgent(live.record);
			this.updateRecord(live, { radiusHavliandAgentId: registeredRecord.radiusHavliandAgentId });
