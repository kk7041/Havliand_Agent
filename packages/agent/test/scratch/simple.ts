...
import { cloudflareAIGatewayProvider } from "@havliand_agent/ai/providers/cloudflare-ai-gateway";
...
const { skills: sourcedSkills } = await loadSourcedSkills<Source, SourcedSkill>(
	env,
	[
		source("project", join(env.cwd, ".havliand_agent/skills")),
		source("user", join(homedir(), ".havliand_agent/agent/skills")),
...
const { promptTemplates: sourcedPromptTemplates } = await loadSourcedPromptTemplates<Source, SourcedPromptTemplate>(
	env,
	[
		source("project", join(env.cwd, ".havliand_agent/prompts")),
		source("user", join(homedir(), ".havliand_agent/agent/prompts")),
	],
