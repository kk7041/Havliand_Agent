# @havliand_agent/agent-core

Stateful agent with tool execution and event streaming. Built on `@havliand_agent/ai`.

## Installation

```bash
npm install @havliand_agent/agent-core
```
...
```typescript
declare module "@havliand_agent/agent-core" {
...
```typescript
import { Agent, streamProxy } from "@havliand_agent/agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@havliand_agent/agent-core";
