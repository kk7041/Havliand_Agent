 ...
 *   --analyze      Spawn HavliandAgent subagents to analyze each transcript file
...
function runSubagent(prompt: string, cwd: string): Promise<{ success: boolean }> {
	return new Promise((resolve) => {
		const child = spawn("havliand_agent", ["--mode", "json", "--tools", "read,write", "-p", prompt], {
			cwd,
		...
		child.on("error", (err) => {
			console.error(chalk.red(`  Failed to spawn havliand_agent: ${err.message}`));
...
async function main() {
	...
	const sessionsBase = join(homedir(), ".havliand_agent/agent/sessions");
	...
	if (!analyzeFlag) {
		console.log("\nRun with --analyze to spawn havliand_agent subagents for pattern analysis.");
		return;
	}

	// Find AGENTS.md files to compare against
	const globalAgentsMd = join(homedir(), ".havliand_agent/agent/AGENTS.md");
