...
const agentDirEnvName = "HAVLIAND_AGENT_CODING_AGENT_DIR";
const startupBenchmarkEnvName = "HAVLIAND_AGENT_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific HAVLIAND_AGENT_CODING_AGENT_DIR for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force HAVLIAND_AGENT_OFFLINE=1 / HAVLIAND_AGENT_SKIP_VERSION_CHECK=1
...
function createBenchmarkEnv(options, isolatedAgentDir) {
	...
	if (options.offline) {
		env.HAVLIAND_AGENT_OFFLINE = "1";
		env.HAVLIAND_AGENT_SKIP_VERSION_CHECK = "1";
	}
	return env;
}
...
async function runTuiBenchmarkRun({ runtime, runIndex, measuredIndex, options, profileDir }) {
	...
	const tempRoot = options.isolatedAgentDir ? mkdtempSync(join(tmpdir(), "havliand_agent-startup-benchmark-")) : undefined;
