...
const packages = [
	{ directory: "packages/ai", name: "@havliand_agent/ai" },
	{ directory: "packages/tui", name: "@havliand_agent/tui" },
	{ directory: "packages/agent", name: "@havliand_agent/agent-core" },
	{ directory: "packages/coding-agent", name: "@havliand_agent/coding-agent" },
];
...
function prepareOutputDirectory(options, repoRoot) {
	if (!options.outDir) {
		return mkdtempSync(join(tmpdir(), "havliand_agent-local-release-"));
	}
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `havliand_agent-${platform}.zip` : `havliand_agent-${platform}.tar.gz`;
	cpSync(join(binaryBuildDirectory, archiveName), join(archiveDirectory, archiveName));
	return platform;
}

function createHavliandAgentShim(installDirectory) {
	const binDirectory = join(installDirectory, "node_modules", ".bin");
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "havliand_agent.cmd"))) {
			writeFileSync(join(installDirectory, "havliand_agent.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\havliand_agent.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "havliand_agent.ps1"), '& "$PSScriptRoot/node_modules/.bin/havliand_agent.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "havliand_agent.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\havliand_agent.exe" %*\r\n');
		writeFileSync(join(installDirectory, "havliand_agent.ps1"), '& "$PSScriptRoot/node_modules/.bin/havliand_agent.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "havliand_agent"), join(installDirectory, "havliand_agent"));
}
...
if (rootPackageJson.name !== "havliand_agent-monorepo") {
...
if (!options.skipInstall) {
	...
	createHavliandAgentShim(nodeInstallDirectory);
	...
	if (!options.skipBunInstall) {
		...
		createHavliandAgentShim(bunInstallDirectory);
	}
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `havliand_agent-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "havliand_agent.exe" : "havliand_agent")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "havliand_agent.cmd" : "havliand_agent")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "havliand_agent.cmd" : "havliand_agent")} --help`);
	}
}
