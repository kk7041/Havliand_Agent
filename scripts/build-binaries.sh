#!/usr/bin/env bash
#
# Build HavliandAgent binaries for all platforms locally.
...
#     havliand_agent-darwin-arm64.tar.gz
#     havliand_agent-darwin-x64.tar.gz
#     havliand_agent-linux-x64.tar.gz
#     havliand_agent-linux-arm64.tar.gz
#     havliand_agent-windows-x64.zip
#     havliand_agent-windows-arm64.zip
...
for platform in "${PLATFORMS[@]}"; do
    ...
    if [[ "$platform" == windows-* ]]; then
        bun build --compile --target=bun-$platform ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/havliand_agent.exe"
    else
        bun build --compile --target=bun-$platform ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/havliand_agent"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        # Windows (zip)
        echo "Creating havliand_agent-$platform.zip..."
        (cd "$platform" && zip -r ../havliand_agent-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating havliand_agent-$platform.tar.gz..."
        mv "$platform" havliand_agent && tar -czf havliand_agent-$platform.tar.gz havliand_agent && mv havliand_agent "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf "$platform"
    if [[ "$platform" == windows-* ]]; then
        mkdir -p "$platform" && (cd "$platform" && unzip -q ../havliand_agent-$platform.zip)
    else
        tar -xzf havliand_agent-$platform.tar.gz && mv havliand_agent "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        echo "  $OUTPUT_DIR/$platform/havliand_agent.exe"
    else
        echo "  $OUTPUT_DIR/$platform/havliand_agent"
    fi
done
